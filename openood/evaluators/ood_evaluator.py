...
class OODEvaluator(BaseEvaluator):
    ...
    def _eval_ood(self,
        ...
        for dataset_name, ood_dl in ood_data_loaders[ood_split].items():
            print(f'Performing inference on {dataset_name} dataset...',
                  flush=True)
            ood_pred, ood_conf, ood_gt, fea = postprocessor.inference(net, ood_dl)

            ood_gt = -1 * np.ones_like(ood_gt)  # hard set to -1 as ood
            if self.config.recorder.save_scores:
                self._save_scores(ood_pred, ood_conf, ood_gt, fea, dataset_name)
    ...
    def _save_scores(self, pred, conf, gt, fea, save_name):
    ...
    def _save_fea(self, fea, save_name):
        save_dir = os.path.join(self.config.output_dir, 'fea')
        os.makedirs(save_dir, exist_ok=True)
        np.savez(os.path.join(save_dir, save_name),
                 fea=fea)
    ...
    def eval_acc(self,
        ...
        if type(net) is dict:
            ...
            net.eval()
        self.id_pred, self.id_conf, self.id_gt, self.fea= postprocessor.inference(
            net, data_loader)
